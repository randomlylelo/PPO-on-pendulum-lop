# https://github.com/Farama-Foundation/Gymnasium/blob/94019feee1a0f945b9569cddf62780f4e1a224a5/gymnasium/envs/classic_control/pendulum.py
...
class PendulumEnv(gym.Env):
    ...
    * v0: Initial versions release
    ...
    def __init__(self, render_mode: Optional[str] = None, g=10.0, target: float = np.pi):
        self.max_speed = 8
        self.max_torque = 2.0
        self.dt = 0.05
        self.g = g
        ...
        self.target = float(target)    # store in radians
    ...
    def step(self, u):
        ...
        # new angle normalize
        # distance to *target* instead of 0 rad
        dist = ((th - self.target + np.pi) % (2*np.pi)) - np.pi
        costs = dist**2 + 0.1 * thdot**2 + 0.001 * u**2
    ...
    def set_target(self, new_angle: float):
        """Change the goal angle (radians, CCW positive)."""
        self.target = float(new_angle)
